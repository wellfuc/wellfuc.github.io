...
SELECT r.id, 'windows', 'x64', a.slug || '-setup.exe', '/var/www/tools/apphub/storage/files/' || a.slug || '-setup.exe', 120000000, md5(random()::text), 'application/octet-stream', 0, NOW()
FROM releases r
JOIN apps a ON a.id = r.app_id
WHERE a.type IN ('desktop','both');

INSERT INTO media (app_id, type, stored_path, caption, sort_order)
SELECT id, 'screenshot', '/var/www/tools/apphub/storage/media/' || slug || '-hero.png', 'Main dashboard view', 1 FROM apps;
