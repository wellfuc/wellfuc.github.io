...
```bash
sudo mkdir -p /var/www/tools/apphub
sudo chown -R $USER:$USER /var/www/tools/apphub
```

### 3) Copy code into place

```bash
rsync -av ./apphub/ /var/www/tools/apphub/
```

### 4) Create virtualenv + install Python packages

```bash
cd /var/www/tools/apphub
python3 -m venv venv
...
- `DATABASE_URL`
- `APP_URL`
- `STORAGE_ROOT=/var/www/tools/apphub/storage`
...
```bash
sudo mkdir -p /var/www/tools/apphub/storage/{files,media}
sudo chown -R www-data:www-data /var/www/tools/apphub/storage
sudo chmod -R 750 /var/www/tools/apphub/storage
```
...
- Files are stored in `/var/www/tools/apphub/storage` and served by nginx via `X-Accel-Redirect`.
...
- Files/media: nightly `rsync` or `tar` of `/var/www/tools/apphub/storage`.

Example:
```bash
pg_dump "$DATABASE_URL" | gzip > /var/backups/apphub-db-$(date +%F).sql.gz
rsync -a /var/www/tools/apphub/storage /var/backups/apphub-storage
```
